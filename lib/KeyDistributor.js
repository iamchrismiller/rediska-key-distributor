...
function KeyDistributor(options) {
  ...
  this._hashring = [];
  //js objects don't maintain order
  this.slicePositionOrder = [];
...
KeyDistributor.prototype.getConnectionsByKeyName = function (name, count) {
  ...
  while (!finished) {
    ...
    for (var ix in this.slicePositionOrder[slice]) {
      //Order Maintained Array
      var position = this.slicePositionOrder[slice][ix];
  ...
  if (Object.keys(this._backends).length < 2) {
    ...
    for (var backend in this._backends) {
      ...
      for (var i = 0; i < weight; i++) {
        ...
        if (!this.slicePositionOrder[slice]) {
          this.slicePositionOrder[slice] = [];
        }

        //Keep track of Object Order
        this.slicePositionOrder[slice].push(position);
    ...
    for (var j = 0; j < this._slicesCount; j++) {
      this.slicePositionOrder[j] = ksort(this._hashring[j], {
        keyOrder    : this.slicePositionOrder[j],
