...
//local
var KeyDistributorException = require('./KeyDistributorException'),
 ...
 */
ConsistentHashRing.prototype.addConnection = function (connectionString, weight) {
  //Does Connection String Exists Already?
  if (this._backends[connectionString]) {
    throw new KeyDistributorException("Error: Connection" + connectionString + " already exists.");
  }

  //Add ConnectionString With Weight To Backends
  this._backends[connectionString] = weight ? weight : this._defaultWeight;
 ...
 * @returns {ConsistentHashRing}
 */
ConsistentHashRing.prototype.removeConnection = function (connectionString) {
  //Get Index Of ConnectionString
  var connection = this._backends[connectionString];
  //Check If Connection Exists in Backend Hashring
  if (!connection) {
    throw new KeyDistributorException("Error: Connection" + connectionString + " does not exist.");
  }
  //Remove from Backend Array
  delete this._backends[connectionString];
 ...
 * @param $name
 */
ConsistentHashRing.prototype.getConnectionByKeyName = function (name) {
...
ConsistentHashRing.prototype.getConnectionsByKeyName = function(name, count) {
  ...
  //Slice of hashring
  var slice = ~~(crc32String / this._slicesDiv) + this._slicesHalf;
...
ConsistentHashRing.prototype._initializeHashring = function() {
...
ConsistentHashRing.prototype._sumOfBackendWeights = function() {
...
ConsistentHashRing.prototype._cleanCache = function() {
  this._cache = [];
  this._cacheCount = 0;
};
